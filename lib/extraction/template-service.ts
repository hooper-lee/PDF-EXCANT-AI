...
async function ensureDefaultExtractionTemplates() {
  await Promise.all(
    DEFAULT_EXTRACTION_TEMPLATES.map((template) =>
      prisma.extractionTemplate.upsert({
...
export async function listExtractionTemplates() {
...
export async function findExtractionTemplateById(templateId: string) {
  await ensureDefaultExtractionTemplates();

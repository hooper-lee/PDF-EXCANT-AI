...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    const decoded = redirect ? decodeURIComponent(redirect) : '';
    if (decoded === '/admin') {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 检查是否有重定向参数（支持 /admin、/checkout 等路径）
      const urlParams = new URLSearchParams(window.location.search);
      const redirect = urlParams.get('redirect') ?? '';
      const decoded = redirect ? decodeURIComponent(redirect) : '';
      const allowedRedirects = ['/checkout', '/dashboard', '/extract'];
      const redirectPath = decoded.startsWith('/') && allowedRedirects.includes(decoded)
        ? decoded
        : '/dashboard';

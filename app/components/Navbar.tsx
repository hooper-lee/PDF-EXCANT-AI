...
export default function Navbar() {
  ...
  try {
    ...
    t = {
      ...
      tools: {
        categories: {
          aiTools: { name: 'AI 提取工具', description: '', badge: '付费' },
          pdfUtils: { name: 'PDF 实用工具', description: '', badge: '免费' },
          convert: { name: '转换与导出', description: '', badge: '免费' },
          edit: { name: '编辑工具', description: '', badge: '免费' },
        },
        toolNames: {
      ...
      <nav className="container mx-auto px-4 py-4">
        <div className="flex items-center justify-between max-w-7xl mx-auto">
          ...
          <div className="flex gap-8 items-center">
              ...
              >
                ...
                <div className="mt-6 pt-4 border-t text-center">
                  <p className="text-xs text-gray-500">
                    <span className="text-green-600 font-medium">{t.tools?.categories?.pdfUtils?.badge ?? ''}</span> {t.navbar?.noLoginRequired ?? ''} · 
                    <span className="text-blue-600 font-medium ml-2">{t.tools?.categories?.aiTools?.name ?? ''}</span> {t.navbar?.aiToolsNeedPro ?? ''}
                  </p>
                </div>
              </div>
            </div>
            ...
            <Link href="/tools" className="text-gray-700 hover:text-blue-600 transition-colors font-medium">{t.navbar?.allTools ?? t.common?.tools ?? 'Tools'}</Link>

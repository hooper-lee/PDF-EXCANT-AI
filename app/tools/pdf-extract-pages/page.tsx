...
export default function PDFExtractPagesPage() {
  ...
  const p = t.tools?.pages?.pdfExtractPages ?? ({} as Record<string, string>);
  ...
  const handleExtract = async () => {
    ...
    try {
      ...
      setToast({ message: (p.successMsg ?? '成功提取 {count} 页！文件已开始下载').replace('{count}', String(pagesToExtract.length)), type: 'success' });
    } catch (error) {
      console.error('提取失败:', error);
      setToast({ message: p.errorMsg ?? '页面提取失败，请检查文件和页面范围', type: 'error' });
    } finally {
      setExtracting(false);
    }
  };
      ...
      <div className="container mx-auto px-4 py-12">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-600 text-lg">{p.subtitle ?? '提取 PDF 指定页面，完全免费'}</p>
          </div>
          ...
          {!file ? (
            <div className="border-2 border-dashed border-gray-300 rounded-2xl p-12 text-center bg-white hover:border-orange-400 transition-all">
              ...
              <p className="text-gray-600 mb-6">{p.uploadDesc ?? '上传需要提取页面的 PDF 文档'}</p>
            ...
            <div className="bg-white rounded-2xl shadow-lg p-8">
              ...
              <div className="mb-6">
                <label className="block text-sm font-medium mb-3">{p.pageRangeLabel ?? '页面范围'}</label>
                <input
                  type="text"
                  value={pageRange}
          ...
          <div className="mt-12 grid md:grid-cols-3 gap-6">
            <div className="bg-white rounded-xl p-6 shadow-md">
              ...
              <h3 className="font-semibold mb-2">{p.card1Title ?? '灵活提取'}</h3>
              <p className="text-sm text-gray-600">{p.card1Desc ?? '支持单页和范围提取'}</p>
            </div>
            <div className="bg-white rounded-xl p-6 shadow-md">
              ...
              <p className="text-sm text-gray-600">{p.card2Desc ?? '原始质量无损提取'}</p>
            </div>
            <div className="bg-white rounded-xl p-6 shadow-md">
              <div className="w-12 h-12 bg-blue-100 rounded-lg flex items-center justify-center mb-4">
                <FileText className="w-6 h-6 text-blue-600" />
              </div>
              <h3 className="font-semibold mb-2">{p.card3Title ?? '快速处理'}</h3>
              <p className="text-sm text-gray-600">{p.card3Desc ?? '快速生成新的 PDF'}</p>
            </div>
          </div>
        </div>
      </div>

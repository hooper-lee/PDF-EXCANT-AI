...
export default function PDFRotatePage() {
  ...
  const p = t.tools?.pages?.pdfRotate ?? ({} as Record<string, string>);
  ...
  const handleRotate = async () => {
    ...
    try {
      ...
      setToast({ message: p.successMsg ?? 'PDF 旋转成功！文件已开始下载', type: 'success' });
    } catch (error) {
      console.error('旋转失败:', error);
      setToast({ message: p.errorMsg ?? 'PDF 旋转失败，请检查文件是否有效', type: 'error' });
    } finally {
      setProcessing(false);
    }
  };
        ...
        <div className="max-w-4xl mx-auto">
            ...
            <div className="bg-white rounded-2xl shadow-lg p-8">
              ...
              <div className="mb-6">
                <label className="block text-sm font-medium mb-3">
                  {p.rotationLabel ?? '旋转角度'}
                </label>
                <div className="grid grid-cols-3 gap-4">
                  {[
                    { value: 90, label: p.rotate90Label ?? '顺时针 90°', icon: '↻' },
                    { value: 180, label: p.rotate180Label ?? '180°', icon: '↻↻' },
                    { value: 270, label: p.rotate270Label ?? '逆时针 90°', icon: '↺' },
                  ].map((option) => (
                    <button
                      key={option.value}
          ...
          <div className="mt-12 grid md:grid-cols-3 gap-6">
            <div className="bg-white rounded-xl p-6 shadow-md">
              ...
              <h3 className="font-semibold mb-2">{p.flexibleTitle ?? '灵活旋转'}</h3>
              <p className="text-sm text-gray-600">
                {p.flexibleDesc ?? '支持 90°、180°、270° 旋转'}
              </p>
            </div>
            <div className="bg-white rounded-xl p-6 shadow-md">
              ...
              <h3 className="font-semibold mb-2">{p.keepQualityTitle ?? '保持质量'}</h3>
              <p className="text-sm text-gray-600">
                {p.keepQualityDesc ?? '旋转后保持原始 PDF 质量'}
              </p>
            </div>
            <div className="bg-white rounded-xl p-6 shadow-md">
              ...
              <h3 className="font-semibold mb-2">{p.quickDownloadTitle ?? '快速下载'}</h3>
              <p className="text-sm text-gray-600">
                {p.quickDownloadDesc ?? '旋转完成后立即下载'}
              </p>
            </div>
          </div>
        </div>

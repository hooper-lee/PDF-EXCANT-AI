...
export default function ExcelToPDFPage() {
  ...
  const p = t.tools?.pages?.excelToPdf ?? {} as Record<string, string>;
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      setToast({ message: p.successMsg ?? 'Excel 转 PDF 成功！文件已开始下载', type: 'success' });
    } catch (error) {
      console.error('转换失败:', error);
      setToast({ message: p.errorMsg ?? 'Excel 转 PDF 失败，请检查文件是否有效', type: 'error' });
    } finally {
      setConverting(false);
    }
  };
      ...
      <div className="container mx-auto px-4 py-12">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-600 text-lg">{p.subtitle ?? '将 Excel 转换为 PDF，完全免费'}</p>
          </div>
          ...
          {!file ? (
            <div className="border-2 border-dashed border-gray-300 rounded-2xl p-12 text-center bg-white hover:border-purple-400 transition-all">
              ...
              <h3 className="text-xl font-semibold mb-2">{p.selectFile ?? '选择 Excel 文件'}</h3>
              <p className="text-gray-600 mb-6">{p.supportedFormats ?? '支持 .xlsx, .xls 格式'}</p>
            ...
            <div className="bg-white rounded-2xl shadow-lg p-8">
              ...
              <div className="mb-6 p-4 bg-blue-50 rounded-lg">
                <h3 className="font-semibold text-blue-900 mb-2">{p.noticeTitle ?? '转换说明'}</h3>
                <ul className="text-sm text-blue-800 space-y-1">
                  <li>• {p.notice1 ?? '支持多个工作表转换'}</li>
                  <li>• {p.notice2 ?? '每个工作表转换为一页 PDF'}</li>
                  <li>• {p.notice3 ?? '大表格会自动调整以适应页面'}</li>
                  <li>• {p.notice4 ?? '超过 40 行的数据会被截断'}</li>
                </ul>
              </div>
          ...
          <div className="mt-12 grid md:grid-cols-3 gap-6">
            <div className="bg-white rounded-xl p-6 shadow-md">
              ...
              <h3 className="font-semibold mb-2">{p.multiSheetTitle ?? '多工作表支持'}</h3>
              <p className="text-sm text-gray-600">
                {p.multiSheetDesc ?? '自动转换所有工作表'}
              </p>
            </div>
            <div className="bg-white rounded-xl p-6 shadow-md">
              ...
              <h3 className="font-semibold mb-2">{p.keepFormatTitle ?? '保持格式'}</h3>
              <p className="text-sm text-gray-600">
                {p.keepFormatDesc ?? '尽可能保持原始表格格式'}
              </p>
            </div>
            <div className="bg-white rounded-xl p-6 shadow-md">
              ...
              <h3 className="font-semibold mb-2">{p.quickConvertTitle ?? '快速转换'}</h3>
              <p className="text-sm text-gray-600">
                {p.quickConvertDesc ?? '快速生成 PDF 文档'}
              </p>
            </div>
          </div>
        </div>

...
export default function PDFWatermarkPage() {
  ...
  const p = t.tools?.pages?.pdfWatermark ?? ({} as Record<string, string>);
  ...
  const handleAddWatermark = async () => {
    ...
    if (watermarkType === 'text' && !watermarkText.trim()) {
      setToast({ message: p.enterText ?? '请输入水印文字', type: 'error' });
      return;
    }
    if (watermarkType === 'image' && !watermarkImage) {
      setToast({ message: p.selectImage ?? '请选择水印图片', type: 'error' });
      return;
    }
    ...
    try {
      ...
      setToast({ message: (p.successMsg ?? '成功为 {count} 页添加水印！文件已开始下载').replace('{count}', String(totalPages)), type: 'success' });
    } catch (error) {
      console.error('添加水印失败:', error);
      setToast({ message: (p.errorMsg ?? '添加水印失败') + (error instanceof Error ? ': ' + error.message : ''), type: 'error' });
    } finally {
      setAdding(false);
    }
  };
      ...
      <div className="container mx-auto px-4 py-12">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-600 text-lg">{p.subtitle ?? '为 PDF 添加文字或图片水印，完全免费'}</p>
          </div>
          ...
          {!file ? (
            <div className="border-2 border-dashed border-gray-300 rounded-2xl p-12 text-center bg-white hover:border-cyan-400 transition-all">
              ...
              <p className="text-gray-600 mb-6">{p.uploadDesc ?? '上传需要添加水印的 PDF 文档'}</p>
            ...
            <div className="bg-white rounded-2xl shadow-lg p-8">
              ...
              <div className="mb-6">
                <label className="block text-sm font-medium mb-3">{p.typeLabel ?? '水印类型'}</label>
                <div className="grid grid-cols-2 gap-4">
                  ...
                  >
                    ...
                    <div className="text-left">
                      <div className="font-semibold">{p.typeText ?? '文字水印'}</div>
                      <div className="text-sm text-gray-600">{p.typeTextDesc ?? '添加文字水印'}</div>
                    </div>
                  </button>
                  <button
                  ...
                  >
                    ...
                    <div className="text-left">
                      <div className="font-semibold">{p.typeImage ?? '图片水印'}</div>
                      <div className="text-sm text-gray-600">{p.typeImageDesc ?? '添加图片水印'}</div>
                    </div>
                  </button>
                </div>
              </div>
              ...
              {watermarkType === 'text' ? (
                <div className="mb-6">
                  <label className="block text-sm font-medium mb-3">{p.textLabel ?? '水印文字'}</label>
                  <input
                    type="text"
                    value={watermarkText}
                    onChange={(e) => setWatermarkText(e.target.value)}
                    placeholder={p.textPlaceholder ?? '输入水印文字'}
                ...
                <div className="mb-6">
                  <label className="block text-sm font-medium mb-3">{p.imageLabel ?? '水印图片'}</label>
                    ...
                    {watermarkImage ? (
                      ...
                      <label className="cursor-pointer">
                        ...
                        <div className="text-gray-600">
                          ...
                          <p>{p.selectImageHint ?? '点击选择图片 (PNG, JPG)'}</p>
                        </div>
                      </label>
                    )}
                  </div>
              ...
              <div className="grid md:grid-cols-2 gap-6 mb-6">
                <div>
                  <label className="block text-sm font-medium mb-3">{(p.opacityLabel ?? '透明度')}: {Math.round(opacity * 100)}%</label>
                  <input
                    type="range"
                    value={opacity}
                ...
                <div>
                  <label className="block text-sm font-medium mb-3">{(p.rotationLabel ?? '旋转角度')}: {rotation}°</label>
                  <input
                    type="range"
                    value={rotation}
          ...
          <div className="mt-12 grid md:grid-cols-3 gap-6">
            <div className="bg-white rounded-xl p-6 shadow-md">
              ...
              <h3 className="font-semibold mb-2">{p.card1Title ?? '文字水印'}</h3>
              <p className="text-sm text-gray-600">{p.card1Desc ?? '支持自定义文字和字体大小'}</p>
            </div>
            <div className="bg-white rounded-xl p-6 shadow-md">
              <div className="w-12 h-12 bg-green-100 rounded-lg flex items-center justify-center mb-4">
                <ImageIcon className="w-6 h-6 text-green-600" />
              </div>
              <h3 className="font-semibold mb-2">{p.card2Title ?? '图片水印'}</h3>
              <p className="text-sm text-gray-600">{p.card2Desc ?? '支持 PNG 和 JPG 图片格式'}</p>
            </div>
            <div className="bg-white rounded-xl p-6 shadow-md">
              <div className="w-12 h-12 bg-purple-100 rounded-lg flex items-center justify-center mb-4">
                <Droplet className="w-6 h-6 text-purple-600" />
              </div>
              <h3 className="font-semibold mb-2">{p.card3Title ?? '自定义样式'}</h3>
              <p className="text-sm text-gray-600">{p.card3Desc ?? '可调节透明度和旋转角度'}</p>
            </div>
          </div>
        </div>
      </div>

...
import { ORDER_STATUS, PAYMENT_METHOD, SUBSCRIPTION_STATUS, USER_PLAN } from '@/lib/domain-types';
...
export async function POST(request: NextRequest) {
  try {
    ...
    // 定义方案信息
    const plans = {
      free: { id: USER_PLAN.FREE, name: '免费版', amount: 0, pages: 300, period: 'lifetime' },
      monthly: { id: USER_PLAN.MONTHLY, name: '专业版', amount: 9.9, pages: 2000, period: 'month' },
      yearly: { id: USER_PLAN.YEARLY, name: '年度版', amount: 107, pages: 20400, period: 'year' }
    };
    ...
    const updatedUser = await prisma.$transaction(async (tx) => {
      ...
      if (planId !== 'free') {
        ...
        await createUsageRecord(
          {
            userId,
            source: USAGE_SOURCE.SUBSCRIPTION_GRANT,
